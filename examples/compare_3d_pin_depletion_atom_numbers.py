#!/usr/bin/env python3
"""Compare dense and tensor-train depletion atom numbers for 3D pin examples."""
...
DEFAULT_DENSE_RESULTS = EXAMPLES / "3d_pin_depletion" / "depletion_results.h5"
DEFAULT_TT_RESULTS = EXAMPLES / "3d_pin_depletion_tt" / "depletion_results.h5"
...
def _check_results_file(path, required_data, regenerate_hint):
    if not path.is_file():
        raise SystemExit(
    ...
    with h5py.File(path, "r") as handle:
        if required_data not in handle:
            raise SystemExit(
                f"{path} does not contain /{required_data}.\n"
                f"{regenerate_hint}")
...
def _compare_step(step, dense_result, tt_reader, nucs, rtol):
    n_values = 0
    n_fail = 0
    max_rel = 0.0
    n_dense_zero = 0
    ...
    for mat in _ordered_keys(dense_result.index_mat):
        ...
        dense_atoms = dense_result.data[dense_mat_index]
        tt_atoms = tt_reader.get_material_atom_vector(step, mat)

        for nuc in nucs:
            dense_nuc_index = dense_result.index_nuc[nuc]
            tt_nuc_index = tt_reader.index_nuc[nuc]

            dense_value = dense_atoms[dense_nuc_index]
            tt_value = tt_atoms[tt_nuc_index]
            if dense_value != 0.0:
                rel_diff = abs(tt_value - dense_value) / abs(dense_value)
    ...
    return (
        n_values, n_fail, max_rel, n_dense_zero,
...
def compare(args):
    _check_results_file(
        args.dense_results,
        "number",
        "Re-run examples/3d_pin_depletion/3d_depletion.py without TT "
        "depletion enabled.",
    )
    _check_results_file(
        args.tt_results,
        "tt_depletion_atom_numbers",
        "Re-run examples/3d_pin_depletion_tt/3d_depletion.py with TT "
        "depletion enabled.",
    )
    ...
    tt_metadata = openmc.deplete.Results(args.tt_results)
    tt_reader = openmc.deplete.TTDepletionAtoms(args.tt_results)

    if len(dense_results) != len(tt_metadata):
        raise SystemExit(
            "The dense and TT result files contain different numbers of steps: "
            f"{len(dense_results)} != {len(tt_metadata)}.")
    if len(tt_metadata) != tt_reader.n_steps:
        raise SystemExit(
            "The TT metadata and TT atom-number data contain different "
            f"numbers of steps: {len(tt_metadata)} != {tt_reader.n_steps}.")
    ...
    dense_nucs = set(first_dense.index_nuc)
    tt_nucs = set(tt_reader.index_nuc)
    if dense_nucs != tt_nucs:
        raise SystemExit(
            "The dense and TT result files contain different atom-number "
            "nuclides.")

    nucs = _ordered_keys(first_dense.index_nuc)
    ...
    for step in steps:
        result = _compare_step(
            step, dense_results[step], tt_reader, nucs, args.rtol)
        (
            n_values, n_fail, max_rel, n_dense_zero,
        ...
        if worst.rel_diff > global_worst.rel_diff:
            global_worst = worst
        ...
        print(
            ...
            f"TT={_keff_value(tt_metadata[step].k):.5f}")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Compare atom numbers from the non-TT and TT 3D pin depletion "
            "examples. The TT depletion_results.h5 file must store atom "
            "numbers in tensor-train format."),
